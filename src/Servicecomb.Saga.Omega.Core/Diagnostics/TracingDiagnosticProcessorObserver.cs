...
namespace Servicecomb.Saga.Omega.Core.Diagnostics
{
    ...
    {
        private  readonly ILogger _logger = LogManager.GetLogger<TracingDiagnosticProcessorObserver>();
        ...
        public void OnNext(DiagnosticListener diagnosticListener)
        {
            ...
            {
                if (diagnosticListener.Name != diagnosticProcessor.ListenerName) continue;
                Subscribe(diagnosticListener, diagnosticProcessor);
                _logger.Debug(
