...
namespace Servicecomb.Saga.Omega.Core.Diagnostics
{
    ...
    {
        private readonly List<TracingDiagnosticMethod> _tracingDiagnosticMethods;

        public TracingDiagnosticMethodCollection(ITracingDiagnosticProcessor tracingDiagnosticProcessor)
        {
            _tracingDiagnosticMethods = new List<TracingDiagnosticMethod>();
            foreach (var method in tracingDiagnosticProcessor.GetType().GetMethods())
            {
                ...
                if (diagnosticName == null)
                    continue;
                _tracingDiagnosticMethods.Add(new TracingDiagnosticMethod(tracingDiagnosticProcessor, method, diagnosticName.Name));
            }
        }
        ...
        {
              return  _tracingDiagnosticMethods.GetEnumerator();
        }
